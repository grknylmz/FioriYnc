...
*/
	...
	initialize: function(saleType) {
		...
		if (dealerAuthType == "B") {
			...
			this.getView().byId("saleGeneralMail").setEditable(false);
...
//yocoskun
		...
		//sprint6-------------
		var oTextDiger = new sap.m.TextArea({
				maxLength: 100,
				rows: 3
			});
		//-------------------
		...
		//--------------------
		//sprint6-------
		if(sTitle === "Diğer Satış Kalemleri" && sSaleType === "H"){
			oFormDetail.addContent(oTextDiger);
		}
		//-----------------
		oFormDetail.addContent(new sap.m.Label({
			text: "Peşin(Nakit/KK)",
			...
			buttons: [
					...
					press: function() {
						...
						var sPriceNew = oInputPrice.getValue().replace(/,/g, ".");
						 var fPriceNew; //tanımlandı sprint5
						//yocoskun sprint5----------------------------
						...
						//sprint6--------------------------------------------------------------------
							if (oTextDiger.getValue().length <= 0 && sTitle == "Diğer Satış Kalemleri") {
							sap.m.MessageToast.show("Fatura açıklama metni giriniz!", {
								at: "center center"
							});
							return;
						...
						//sprint6------------------------------------------------------
							...
							if (oTextDiger.getValue()) {
								oItem.setSecondStatus(new sap.m.ObjectStatus({
									text: oTextDiger.getValue(),
									state: sap.ui.core.ValueState.Success
								}));
							} 
						}
	...
	addProduct: function(oData, oList, oController) {
		...
		//    else add new
			...
			if ((sSaleType == "S" && ((bTTG && aItem.length >= 4) || (!bTTG && aItem.length >= 6)))) {
				//	(sSaleType == "H" && aItem.length >= 6)) { ---yocoskun 27122016
				var oMessageData = {
					TYPE: "E",
	...
	cashOrder: function(oEvent) {
	  ...
	  xmlDoc = parser.parseFromString(data,"text/xml");
		...
		for (var i = 0; i < aItem.length; i++) {
			...
			sDigerTitle = aItem[i].getTitle(); //sprint6
			...
			// diğer metni
				if (aItem[i].getSecondStatus()) {
				if(sSaleType == "H" && sDigerTitle == "Diğer Satış Kalemleri"){
					sDiger = aItem[i].getSecondStatus().getText().split(" - ")[0];
				}
				else{
					sDiger = "";
				}	
	...
	customerTypeSelected: function(oEvent) {
		...
		switch (oView.byId("generalSaleCustomerType").getSelectedIndex()) {
			...
			case 1:
				...
				oView.byId("generalCompanyType").setVisible(true); // Agile Sprint3 yocoskun
